...
void initTimer1(int numhz){
    cli(); //disble global interrupts

    // Reset timer2 to a blank condition
    // TCCR = Timer/Counter control Register
    TCCR1A = 0; // set enture TCCE3A register to 0
    TCCR1B = 0; // set entire TCCR3B register to 0
    ...
    TCCR1B = TCCR1B | bitnumber; // all clock select bits on the end of TCCR3B

    // set compare match register to desired timer count.
    OCR1A = (unsigned int) ((16e6/prescale)/numhz);
